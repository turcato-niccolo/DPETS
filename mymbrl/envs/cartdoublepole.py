...
class CartDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(self):
        ...
        # Initial configuration: both links pointing down
        self.init_qpos = np.array([0.0, np.pi, 0.0])  # [cart pos, theta1, theta2]
    ...
    def step(self, a):
        ...
        target = np.array([0.0, sum(CartDoublePendulumEnv.PENDULUM_LENGTH)])
    ...
    def _get_ee_pos(x):
        ...
        l1, l2 = CartDoublePendulumEnv.PENDULUM_LENGTH
    ...
    def obs_cost_fn_cost(obs):
        ...
        if isinstance(obs, np.ndarray):
            return -np.exp(-np.sum(
                np.square(CartDoublePendulumEnv._get_ee_pos_cost(obs) - target), axis=1
            ) / (sum(CartDoublePendulumEnv.PENDULUM_LENGTH) ** 2))
        else:
            target_t = torch.tensor(target, device=obs.device)
            return -torch.exp(-torch.sum(
                torch.square(CartDoublePendulumEnv._get_ee_pos_cost(obs) - target_t), dim=1
            ) / (sum(CartDoublePendulumEnv.PENDULUM_LENGTH) ** 2))
